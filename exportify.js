...
const utils = {
	...
	async apiCall(url, delay = 0) {
		...
		if (response.ok) { return response.json() }
			...
			if (!error.innerHTML.includes("fa-bolt")) {
				error.innerHTML += '<p><i class="fa fa-bolt" style="font-size: 50px; margin-bottom: 20px">\
...
class PlaylistTable extends React.Component {
	...
	async init() {
		...
		let liked_songs = {
			name: "Liked Songs", external_urls: { spotify: "https://open.spotify.com/collection/tracks" },
	...
	sortRows(column) {
		...
		this.setState({
			playlists: this.state.playlists.sort((a, b) => // make sure to use setState() so React reacts! Calling render() doesn't cut the mustard.
				arrow.className.endsWith("desc") ? // figure out whether we're ascending or descending
					...
					column == "Tracks" ? b.tracks.total - a.tracks.total : field(b).localeCompare(field(a)))
...
// Handles exporting playlists as files
	...
	if (code) {
		...
		let response = await fetch("https://accounts.spotify.com/api/token", {
			...
			body: new URLSearchParams({
				client_id: "d99b082b01d74d61a100c9a0e056380b", grant_type: 'authorization_code', code: code, redirect_uri: location.origin + location.pathname,
				code_verifier: localStorage.getItem('code_verifier')
			})
		}) // POST to get the access token, then fish it out of the response body
